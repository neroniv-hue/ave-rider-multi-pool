/**
 * 🌊 WAVE RIDER MULTI-ASSET PORTFOLIO RISK ENGINE (EXACT ON-CHAIN FIXED RATIOS)
 */
"use strict";
...
const assetPools = [
    { name: "WPLS/USDC", contract: "0xe56043671df55de5cdf8459710433c10324de0ae", currentPrice: 0.00001218, baseline: 0.00001210, rollingWindow: [], position: null, dec0: 18, dec1: 6, inverse: false },
    { name: "PLSX/WPLS", contract: "0x149b2c2d2cb2fbf23bb1d0b30bb224ba46066f9f", currentPrice: 0.00000041, baseline: 0.00000040, rollingWindow: [], position: null, dec0: 18, dec1: 18, inverse: true },
    { name: "HEX/WPLS",  contract: "0xf1f4ee610b2babb05c635f726ef8b0c568c8dc65", currentPrice: 0.00359000, baseline: 0.00355000, rollingWindow: [], position: null, dec0: 18, dec1: 18, inverse: true },
    { name: "DAI/WPLS",  contract: "0xefd766ccb38eaf1dfd701853bfce31359239f305", currentPrice: 1.00000000, baseline: 0.99800000, rollingWindow: [], position: null, dec0: 18, dec1: 18, inverse: false },
    { name: "INC/WPLS",  contract: "0xf808bb6265e9ca27002c0a04562bf50d4fe37eaa", currentPrice: 0.58400000, baseline: 0.58000000, rollingWindow: [], position: null, dec0: 18, dec1: 18, inverse: false }
];
...
async function renderHtmlLayout() {
    // 1. Establish precise global dollar anchor from WPLS pool contract
    ...
    // 2. Clear out mixed logic blocks and route pools cleanly via absolute array definitions
    for (let i = 0; i < assetPools.length; i++) {
        ...
        if (reserves && reserves.r0 > 0 && reserves.r1 > 0) {
            // Find raw structural pool ratio depending on token order layout indexes
            let ratio = pool.inverse ? (reserves.r0 / reserves.r1) : (reserves.r1 / reserves.r0);
            
            // Multiply ratio against our real-time master WPLS dollar anchor to scale fiat value
            let finalizedPrice = ratio * wplsUsdPrice;
