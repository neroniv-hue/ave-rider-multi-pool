/**
 * 🌊 WAVE RIDER MULTI-ASSET PORTFOLIO RISK ENGINE (FIXED ON-CHAIN FIAT CONVERSIONS)
 */
"use strict";
...
const assetPools = [
    { name: "WPLS/USDC", contract: "0xe56043671df55de5cdf8459710433c10324de0ae", currentPrice: 0.00001218, baseline: 0.00001210, rollingWindow: [], position: null, type: "DIRECT" },
    { name: "PLSX/WPLS", contract: "0x149b2c2d2cb2fbf23bb1d0b30bb224ba46066f9f", currentPrice: 0.00001000, baseline: 0.00000990, rollingWindow: [], position: null, type: "WPLS_BASE" },
    { name: "HEX/WPLS",  contract: "0xf1f4ee610b2babb05c635f726ef8b0c568c8dc65", currentPrice: 0.00274000, baseline: 0.002710, rollingWindow: [], position: null, type: "WPLS_BASE" },
    { name: "DAI/WPLS",  contract: "0xe56043671df55de5cdf8459710433c10324de0ae", currentPrice: 1.00000000, baseline: 0.99800000, rollingWindow: [], position: null, type: "WPLS_QUOTE" },
    { name: "INC/WPLS",  contract: "0xf808bb6265e9ca27002c0a04562bf50d4fe37eaa", currentPrice: 0.57000000, baseline: 0.56500000, rollingWindow: [], position: null, type: "WPLS_QUOTE" }
];
...
function processMultiAssetEngine(p, nextPrice) {
    ...
    if (p.position && p.position.type === 'LONG') {
        ...
        if (nextPrice >= p.position.floor * 1.035 || p.position && nextPrice >= p.position.entry * 1.015) { totalWplsWallet += (p.position.sizeTokens - ((p.position.sizeUsdc / nextPrice) * 1.003)); globalLedgerLogs.push({ pair: p.name, action: 'COVER SHORT', price: nextPrice }); p.position = null; p.baseline = nextPrice; }
    }
}

async function renderHtmlLayout() {
    // 1. First Pass: Resolve clean WPLS dollar worth to serve as cross-rate anchor
    let wplsUsdPrice = 0.00001218;
    const wplsReserves = await queryOnChainReserves("0xe56043671df55de5cdf8459710433c10324de0ae");
    if (wplsReserves) { wplsUsdPrice = (wplsReserves.r1 / wplsReserves.r0) * 1e12; }

    // 2. Second Pass: Loop through pools using true dollar logic paths
    for (let i = 0; i < assetPools.length; i++) {
        const pool = assetPools[i];
        if (pool.name === "WPLS/USDC") {
            pool.currentPrice = wplsUsdPrice;
            processMultiAssetEngine(pool, wplsUsdPrice);
            continue;
        }
        ...
        if (reserves && reserves.r0 > 0 && reserves.r1 > 0) {
            ...
            if (pool.type === "WPLS_BASE") {
                // Ratio calculation multiplied by anchor value (e.g. PLSX or HEX)
                conversionPrice = (reserves.r1 / reserves.r0) * wplsUsdPrice;
            } else if (pool.type === "WPLS_QUOTE") {
                // Inverted configuration calculations scaled back cleanly (e.g. DAI or INC)
                conversionPrice = (reserves.r0 / reserves.r1) * wplsUsdPrice;
            }
            pool.currentPrice = conversionPrice;
            processMultiAssetEngine(pool, conversionPrice);
        }
    }
    ...
    let lRows = globalLedgerLogs.length === 0 ? '<tr><td colspan="4">No trades logged yet inside this tracking pass.</td></tr>' : ''; [...globalLedgerLogs].reverse().slice(0, 5).forEach(l => { let dec = l.price < 0.01 ? 6 : 4; lRows += `<tr><td><b>${l.action}</b></td><td>${l.pair}</td><td>$${l.price.toFixed(dec)}</td><td>Verified</td></tr>`; });
...
http.createServer(async (req, res) => { res.writeHead(200, { 'Content-Type': 'text/html' }); res.end(await renderHtmlLayout()); }).listen(process.env.PORT || 10000, () => { console.log("📡 Multi-Asset Dollar Conversion Engine Active"); });
